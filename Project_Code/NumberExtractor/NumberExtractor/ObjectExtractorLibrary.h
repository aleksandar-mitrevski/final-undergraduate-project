...
void ObjectExtractorLibrary::ExtractAndSaveObjects()
{
	...
	//we discard the images with all white pixels because
	//there is no chance that they have a character
	i = 0;
	while(i < numberOfExtractedObjects)
	{
		int blackPixels = numberOfImagePixels - countNonZero(objects[i]);
		if(blackPixels == 0)
		{
	...
	//{
	//	string fileName = "C:/Users/aleksandar_mitrevski/Documents/NxtPath/opencvImages/character " + std::to_string(i) + ".jpg";
...
vector<RowObjects> ObjectExtractorLibrary::ExtractHorizontalBoundaries(Mat binaryImage)
{
	...
	//we found an upper row boundary; if, on the other hand,
	//we don't find white pixels, while 'boundaryFound' is true,
	//then we found a lower boundary
	for(int row=0; row<binaryImage.rows; row++)
	{
		...
		//if we found a white pixel, we check if we already have an upper boundary
		if(whitePixelFound)
		{
	...
	{
		if(boundaries[i].RowBoundaries.LowerBoundary < 0)
...
void ObjectExtractorLibrary::ExtractVerticalObjectBoundaries(Mat binaryImage, vector<RowObjects>& boundaries, unsigned int currentRow)
{
	...
	for(int column=0; column<binaryImage.cols; column++)		
	{
		...
		//if we found a white pixel, we check if we already have a left boundary
		if(whitePixelFound)
		{
...
void ObjectExtractorLibrary::ExtractHorizontalObjectBoundaries(Mat binaryImage, vector<RowObjects>& boundaries, unsigned int currentRow)
{
	...
	if(boundaries[currentRow].Objects[boundaries[currentRow].Objects.size() - 1].LowerBoundary < 0
		|| boundaries[currentRow].Objects[boundaries[currentRow].Objects.size() - 1].LowerBoundary > binaryImage.cols)
	{
